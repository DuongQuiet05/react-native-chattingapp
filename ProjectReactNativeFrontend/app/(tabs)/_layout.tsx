...
import {
  useUnreadCount,
  useUnreadMessageNotificationCount,
...
export default function TabLayout() {
  ...
  const unreadMessageNotificationCountQuery =
    useUnreadMessageNotificationCount(); // Badge cho tin nhắn từ notification API
  ...
  const friendRequestCount =
    status === "authenticated" ? friendRequestCountQuery.data : undefined;
  const unreadCount =
    status === "authenticated" ? unreadCountQuery.data : undefined;
  const unreadMessageNotificationCount =
    status === "authenticated"
      ? unreadMessageNotificationCountQuery.data
      : undefined;
  // Use unread messages count from conversations (real-time) for tab bar badge
  const unreadMessagesCount =
    status === "authenticated" ? unreadMessagesCountQuery.data : undefined;
  const hasRedirectedRef = useRef(false);
  const previousStatusRef = useRef<
    "loading" | "unauthenticated" | "authenticated" | undefined
  >(undefined);
  ...
  return (
    ...
    >
      ...
      <Tabs.Screen
        name="index"
        options={{
          href: null, // Ẩn khỏi tab bar - đã chuyển vào header feed
        }}
      />
      <Tabs.Screen
        name="contacts"
